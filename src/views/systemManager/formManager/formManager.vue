    ...
    methods: {
      ...
      deleteList() {
        this.$confirm('确定删除?', '提示', {
          ...
          del('/Form/DeleteFormDesign', this.multipleSelection).then(response => {
            this.getList()
            this.$message({
              type: 'success',
              message: '删除成功!'
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            // 需要验证的输入框验证通过后才能到这一步
            if (!this.myForm.id) {
              post('/Form/AddFileGroup', this.myForm).then(response => {
                this.getList()
                this.$message({
                  type: 'success',
                  message: '新增成功!'
              ...
              put('/Form/EditFormDesign', this.myForm).then(response => {
